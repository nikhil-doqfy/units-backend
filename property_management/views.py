...
from user_service.models import UserProfile,Documents,OwnerDocumentsMapping,StaffDocumentsMapping,CompanyUserDocumentsMapping,TenantDocumentsMapping,PropertyUnitDetails,Property,Company,PMStaffCompanyMapping,PropertyImages
...
# from property_management.utils import get_property_images , get_tenant_data ,get_lease_ejari_documents
import math
...
def owner_details_list_view(request):
    try:
        current_user = request.user.profile  # UserProfile

        # Only company user allowed
        if current_user.user_role != constants.COMPANY_USER:
            return prepare_response(
                message="Only company users can access this.",
                status=status.HTTP_403_FORBIDDEN
            )

        company = Company.objects.filter(company_user=current_user).first()
        ...
        if request.method == "GET":
            ...
            # All owners having property under this company
            ...
            if owner_id:
                ...
                if rental_status_filter == constants.AVAILABLE:
                    properties = properties.filter(is_occupied=False)
                elif rental_status_filter == constants.NOT_AVAILABLE:
                ...
                for p in properties:
                    ...
                    tenant_image = tenant.profile_image if tenant and tenant.profile_image else None
                    tenant_name = (
                        f"{tenant.user.first_name} {tenant.user.last_name}"
                        if tenant else None
                    )

                    # Property Images
                    images_qs = PropertyImages.objects.filter(property=p)
                    images_list = []
                    ...
                    property_with_tenant_data.append({
                        ...
                        "property_name": p.property_unit_name,
                        "tenancy_status": (
                            constants.NOT_AVAILABLE if p.is_occupied else constants.AVAILABLE
                        ),
                        "images": images_list if images_list else None,
                        "tenant": {
                            "name": tenant_name,
                            "profile_image": tenant_image,
...
def pmc_owner_view_list(request):
    try:
        ...
        # -----------------------------------------------------------
        # 🔹 PUT REQUEST → Assign PMC to Property
        # -----------------------------------------------------------
        if request.method == "PUT":
            body = json.loads(request.body)
            property_id = body.get("property_id")
            pmc_company_id = body.get("pmc_id")

            # Required fields
            if not property_id or not pmc_company_id:
                return prepare_response(
                    message=constants.REQUIRED_PMC_PROPERTY,
                    status=status.HTTP_400_BAD_REQUEST
                )

            # Validate property belongs to this owner
            property_obj = PropertyUnitDetails.objects.filter(
                id=property_id,
                owner=current_user
            ).first()
            ...
            if not property_obj:
                return prepare_response(
                    message=constants.YOU_NOT_OWNER_OF_PROPERTY,
                    status=status.HTTP_403_FORBIDDEN
                )

            # Find company (PMC)
            pmc_company = Company.objects.filter(id=pmc_company_id).select_related("company_user").first()
            ...
            # Assign PMC (company)
            property_obj.company = pmc_company
            property_obj.save(update_fields=["company"])

            return prepare_response(
                message=constants.PROPERTY_ASSIGNED_SUCCESS,
                status=status.HTTP_200_OK
            )

        # -----------------------------------------------------------
        # 🔹 GET REQUEST → Fetch PMC list for this owner
        # -----------------------------------------------------------
        if request.method == "GET":

            # 1️⃣ Get all properties owned by this owner
            owner_properties = PropertyUnitDetails.objects.filter(
                owner=current_user
            ).select_related("company")

            if not owner_properties.exists():
                return prepare_response(
            ...
            # 2️⃣ Get all unique PMC companies
            companies = Company.objects.filter(
                properties__owner=current_user
            ).distinct().select_related("company_user")

            pmc_list = []

            # 3️⃣ For each PMC company
            for company in companies:
                pmc = company.company_user  # UserProfile of PMC

                # Documents
                pmc_docs = CompanyUserDocumentsMapping.objects.filter(company_user=pmc)

                pmc_doc_list = []
                for doc_map in pmc_docs:
                    url = fetch_s3_presigned_url(
                        doc_map.document.file_path,
                        doc_map.document.file_name
                    )
                    pmc_doc_list.append({
                        "file_name": doc_map.document.file_name,
                        "file_type": doc_map.document.document_choice,
                        "url": url,
                    })

                # PMC properties for this owner
                properties_under_pmc = PropertyUnitDetails.objects.filter(
                    company=company,
                    owner=current_user
                )

                property_list = []
                for prop in properties_under_pmc:
                    property_list.append({
                        "property_id": prop.id,
                        "property_name": prop.property_unit_name,
                        "property_code": prop.property_code,
                        "rental_status": constants.NOT_AVAILABLE if prop.is_occupied else constants.AVAILABLE
                    })

                pmc_list.append({
                    "pmc_id": pmc.id,
                    "pmc_name": f"{pmc.user.first_name} {pmc.user.last_name}",
                    "pmc_email": pmc.user.email,
                    "pmc_contact": pmc.contact_number,
                    "pmc_profile_image": pmc.profile_image,
                    "company_id": company.id,
                    "company_name": company.company_name,
                    "company_code": company.company_code,
                    "documents": pmc_doc_list,
                    "properties_handled": property_list
                })

            return prepare_response(
                message=constants.DATA_FETCHED,
                data={"pmc_list": pmc_list},
                status=status.HTTP_200_OK
            )
...
def property_details_list_view(request):
    ...
    try:
        user = request.user.profile  # Current user profile
        ...
        # Base queryset
        properties = PropertyUnitDetails.objects.select_related(
            "owner",
            "company",
            "property"
        ).order_by("-id")
        ...
        # Filter based on user type (owner, staff, company user)
        if not property_id:
            ...
            if not all_properties:
                if user.user_role == constants.OWNER:
                    ...
                    properties = properties.filter(company__company_user=user)
                elif user.user_role == constants.STAFF:
                    properties = properties.filter(assigned_staff__user_profile=user).distinct()
        ...
        for prop in properties_page:
            ...
            owner = prop.owner
            owner_info = {
                "owner_name": f"{owner.user.first_name} {owner.user.last_name}" if owner else "N/A",
                "contact_number": owner.contact_number if owner else "N/A",
                "trade_license": owner.trade_license_number if owner else "N/A",
                "email": owner.user.email if owner else None,
                "profile_image": owner.profile_image if owner else None,
                "emirate_id": owner.emirate_id if owner else "N/A",
                "uae_residence_visa": owner.uae_residence_visa if owner else "N/A",
                "owner_code": getattr(owner, "owner_number", "N/A") if owner else "N/A",
            }
            ...
            pmc_info = {
                ...
                "contact_number": pmc.company_user.contact_number if pmc and pmc.company_user else "N/A",
                "email": pmc.company_user.user.email if pmc and pmc.company_user else None,
                "profile_image": pmc.company_user.profile_image if pmc and pmc.company_user else None,
            }

            # Commercial info (from property unit fields)
            commercial_info = {
                "rent": prop.rent,
            ...
            rental_status_display = constants.RENTAL_NOT_AVAILABLE if prop.is_occupied else constants.RENTAL_AVAILABLE
        ...
        pagination_meta = None if property_id else {
...
def create_property_basic(request):
    ...
    if request.method == "GET":
        try:
            ...
            property_data = {
                ...
                "value": property_type_options.get(prop.property_type)
            }
    ...
    # ------------------- PUT Method -------------------
        ...
        try:
            ...
            updatable_fields = [
                "property_unit_name", "land_dm_no", "address", "area_of_property",
                "no_of_parking", "makani_no", "dewa_no", "property_type",
                "land_area", "apartment_no", "bedrooms", "balcony",
                "plot_no", "area_unit", "land_area_unit", "apartment_floor_no",
                "no_of_floors", "property_code"
            ]
    ...
    # ------------------- POST Method -------------------
        ...
        try:
            ...
            # Determine owner and company based on user role
            ...
            # Default property object (create parent property if needed)
            ...
            new_property_unit = PropertyUnitDetails.objects.create(
                property_unit_name=property_name,
...
def add_commercial_details(request):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            prop_unit = PropertyUnitDetails.objects.filter(id=property_id).first()
            if not prop_unit:
                return prepare_response(
                    message=constants.PROPERTY_NOT_FOUND,
                    status=status.HTTP_404_NOT_FOUND
            ...
            data = {
                ...
                "rent": prop_unit.rent,
                "security_deposit": prop_unit.security_deposit,
                "booking_amount": prop_unit.booking_amount,
                "maintenance_charges": prop_unit.maintenance_charges,
                "cycle": prop_unit.cycle,
                "notice_period": prop_unit.notice_period,
                "commission_percent": prop_unit.commission_percent,
                "pmc": {
                    "key": prop_unit.company.id,
                    "value": prop_unit.company.company_name
                } if prop_unit.company else None,
                "step_choice": prop_unit.step_status
            }

            return prepare_response(
                content=data,
                message=constants.COMMERCIAL_DETAILS_FETCHED,
                status=status.HTTP_200_OK
            )
            ...
            if user_profile.user_role == constants.OWNER:
                ...
                if pmc_id:
                    company_obj = Company.objects.filter(id=pmc_id).first()
                    if not company_obj:
                        return prepare_response(
                            message=constants.PMC_NOT_FOUND,
                            status=status.HTTP_400_BAD_REQUEST
                        )
                    prop_unit.company = company_obj
                    prop_unit.save()
            ...
            for field in decimal_fields:
                if field in data:
                    setattr(prop_unit, field, safe_decimal(data[field]))
            for field in other_fields:
                if field in data:
                    setattr(prop_unit, field, data[field])

            prop_unit.step_status = "COMMERCIALS_DETAILS"
            prop_unit.save()

            message = constants.COMMERCIAL_DETAILS_UPDATED if request.method == "PUT" else constants.COMMERCIAL_DETAILS_CREATED
...
#             return prepare_response(
#                 message=constants.NO_PROPERTY_FOUND_OWNER,
...
#                 if not pmc_obj:
#                     return prepare_response(
#                         message=constants.PMC_LINKED_NOT,
#                         status=status.HTTP_404_NOT_FOUND
#                     )
...
#                 return prepare_response(message=constants.INVALID_LEASE_DATE, status=status.HTTP_400_BAD_REQUEST)
...
#             docs=get_lease_ejari_documents(lease_id)
#             if tenant_data:
#                 return prepare_response(
#                     content={"tenant_details": tenant_data,"lease_documents": docs if docs else []},
