...
def convert_lead_to_tenant(request):
    if request.method == "GET":
        ...
        if lead.status == constants.LEASE_TENANCY:
            return prepare_response(
                message=constants.LEAD_ALREADY_CONVERTED,
                status=status.HTTP_400_BAD_REQUEST
            )

        existing_tenant = UserProfile.objects.filter(
            user__email=lead.email,
            is_active=True
        ).first()

        if existing_tenant:
            lead.tenant = existing_tenant
        else:
            django_user = User.objects.create_user(
                username=lead.email,
                email=lead.email,
            ...
            new_tenant = UserProfile.objects.create(
                user=django_user,
            ...
            lead.tenant = new_tenant

        lead.status = constants.LEASE_TENANCY
        lead.save()

        return prepare_response(
            message=constants.LEAD_CONVERTED_SUCCESSFULLY,
            status=status.HTTP_200_OK
        )
