...
from user_service.models import UserProfile,Documents,OwnerDocumentsMapping,  CompanyUserDocumentsMapping,TenantDocumentsMapping , Company,Country, State, City , Role, PropertyUnitDetails, Lead
...
def create_lead(request):
    ...
    if request.method == "POST":
        ...
        unit = None
        if unit_id:
            unit = PropertyUnitDetails.objects.filter(id=unit_id,is_active=True ).first()
            if not unit:
                return prepare_response(
