...
from user_service.models import UserProfile, Documents, OwnerDocuments, TenantDocuments, Role, Owner, Tenant, PropertyManager ,Approval, DocumentType, Documentation, AssignedUnit
...
def unit_assign(request):
    ...
    if request.method=="GET":
        ...
        for pm in pms:
            ...
            assignments=pm.user_assigned_units.all()
        ...
        if unit_assigned is not None:
            pm.user_assigned_units.exclude(unit_id__in=unit_assigned).delete()
            for unit_id in unit_assigned:
                unit=Unit.objects.filter(id=unit_id).first()
                if unit:
                    AssignedUnit.objects.get_or_create(
