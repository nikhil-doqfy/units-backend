...
class Unit(Base):
    ...
    owner = models.ForeignKey("user_service.UserProfile", on_delete=models.SET_NULL, limit_choices_to={'user_role': constants.OWNER}, related_name="owner_properties",null=True,blank=True
    )
    company = models.ForeignKey(Company,on_delete=models.CASCADE, related_name="company_units", null=True, blank=True)
    assigned_staff = models.ManyToManyField("user_service.CompanyStaff",related_name="assigned_units",blank=True)
...
class PropertyImages(Base):
    property = models.ForeignKey(Property,on_delete=models.CASCADE,related_name="property_images",null=True,blank=True)
    image_path = models.TextField(null=True, blank=True)
    image_type = models.CharField(max_length=20,choices=constants.IMAGE_TYPE_CHOICES,default="INTERIOR",null=True,blank=True)
...
class PropertyInterest(Base):
    property_unit = models.ForeignKey( Property,on_delete=models.CASCADE,related_name="interests")
    tenant = models.ForeignKey("user_service.UserProfile",on_delete=models.CASCADE,related_name="interested_properties")
...
class PropertyDocumentsMapping(Base):
    ...
    property = models.ForeignKey(Property,on_delete=models.CASCADE, related_name="property_documents",null=True, blank=True)
    document = models.ForeignKey("user_service.Documents",on_delete=models.CASCADE,related_name="property_document_mappings",null=True,blank=True)
    document_choice = models.CharField( max_length=50,choices=PROPERTY_DOCUMENT_CHOICES,default=constants.FLOOR_PLAN)
...
class CompanyUserDocumentsMapping(Base):
    ...
    document = models.ForeignKey("user_service.Documents",on_delete=models.CASCADE,related_name="company_user_document_mappings",null=True,blank=True)
    document_choice = models.CharField( max_length=50,choices=COMPANY_DOCUMENT_CHOICES,default=constants.EMIRATES_ID)
